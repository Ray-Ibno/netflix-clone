...
export const fetchTvTrailers = async (tvId) => {
  const data = await fetchTmdb(`https://api.themoviedb.org/3/tv/${tvId}/videos`)
  if (!data || data.results.length === 0) throw new AppError('no data found', 404)
  return data.results
}

export const fetchTvDetails = async (tvId) => {
  const data = await fetchTmdb(`https://api.themoviedb.org/3/tv/${tvId}`)
  if (!data) throw new AppError('no data found', 404)
  return data
}

export const fetchSimilarTvShows = async (tvId) => {
  const data = await fetchTmdb(`https://api.themoviedb.org/3/tv/${tvId}/similar`)
  if (!data) throw new AppError('no data found', 404)
  return data.results
}
